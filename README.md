...
Talking repo structure: Each folder contains a writeup on each memory type, to access a write-up for a specific memory cell access the .md file
...
|---|---|---|---|
| Sense Amplifiers | Three topologies implemented and compared: gated diode (cross-coupled), voltage latch, and current mirror sense amplifier; analysis covers input-referred offset, regeneration speed, and noise sensitivity; topology selection rationalized against SRAM read SNM budget and bitline swing requirements | Cadence Virtuoso | gpdk045 |
